...
/* Layout: [sizeof(CnromState)] struct + [chr_size] CHR (only if chr_is_ram). */
static size_t cnrom_save_state(const void* state, uint8_t* buf) {
    const CnromState* s = (const CnromState*)state;
    size_t total = sizeof(CnromState);
    if (s->chr_is_ram) {
        total += s->chr_size;
    }
    if (buf) {
        memcpy(buf, s, sizeof(CnromState));
        if (s->chr_is_ram && s->chr_size > 0u) {
            memcpy(buf + sizeof(CnromState), s->chr, s->chr_size);
        }
    }
    return total;
}

static bool cnrom_load_state(void* state, const uint8_t* buf, size_t len) {
    CnromState* s = (CnromState*)state;
    size_t need = sizeof(CnromState);
    if (s->chr_is_ram) {
        need += s->chr_size;
    }
    if (len < need) {
    ...
    memcpy(s, buf, sizeof(CnromState));
    s->prg_rom = valid_prg;
    ...
    if (s->chr_is_ram && s->chr_size > 0u) {
        memcpy(s->chr, buf + sizeof(CnromState), s->chr_size);
    }
    return true;
}
